...
class Drone:
    ...
    def load(self, arm, box, duration = 1, cost = 1):
        if arm is None:
            raise ValueError(f"No arm was passed as parameter, in order to load the box {box.name}, an arm is needed")
    ...
    # el dron y el transportador estén en la misma ubicación, y que el brazo esté vacío.
    #
